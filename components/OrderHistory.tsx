...
import { ChevronDown, ChevronRight, Gift, Edit, CheckCircle, XCircle } from 'lucide-react';
...
const OrderHistory: React.FC = () => {
  ...
  return (
    <>
      {statusMessage && (
        <div className={`mb-4 flex items-center p-3 rounded-md ${statusMessage.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
                    ...
                    <td className="p-3">
                        {order.status === OrderStatus.PENDING && (
                            <div className="flex gap-2">
                                ...
                                <Button size="sm" variant="primary" className="bg-green-600 hover:bg-green-700" onClick={() => handleMarkDelivered(order.id)} isLoading={updatingOrderId === order.id} disabled={!!updatingOrderId}><CheckCircle size={14} className="mr-1"/> Deliver</Button>
                            </div>
                        )}
                    </td>
                  </tr>
          ...
          {orders.length === 0 && !loading && <p className="text-center p-4 text-black">No orders have been placed yet.</p>}
        </div>
      </Card>
      {editingOrder && (
          <EditOrderModal
